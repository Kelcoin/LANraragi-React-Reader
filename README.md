...
启用超分后，模型、自动启用和阈值设置才可编辑。自动模式按归档每页平均体积与阈值判断是否启用当前档案；沉浸模式中的超分按钮会在图片过大、不适合推理时隐藏。原图会先正常显示，超分完成后才替换，失败时保留原图。超分输出会在必要时降采样到不超过 3200 万像素，以降低后续解码压力；单次推理输出超过 6400 万像素时会跳过超分。
